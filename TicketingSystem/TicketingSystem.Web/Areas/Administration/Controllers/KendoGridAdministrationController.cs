namespace TicketingSystem.Web.Areas.Administration.Controllers
{
    ...
    {
        public KendoGridAdministrationController(ITicketSystemData data)
            : base(data)
        {
        }
        ...
        protected abstract T GetById<T>(object id)
            where T : class;
        ...
        [NonAction]
        protected virtual T Create<T>(object model)
            where T : class
        {
            if (model != null && ModelState.IsValid)
            {
                var dbModel = AutoMapperConfig.Configuration
               .CreateMapper()
               .Map<T>(model);
            ...
            where TViewModel : class
        {
            if (model != null && ModelState.IsValid)
            {
               ...
               .CreateMapper()
               .Map<TViewModel, TModel>(model, dbModel);
